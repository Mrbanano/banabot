...
from banobot import __version__, __logo__
...
def onboard():
    ...
    if typer.confirm("🍌 ¿Configurar Banabot ahora? / Configure Banabot now?", default=True):
        from banobot.cli.config_wizard import config_wizard
        config = load_config()
...
def _make_provider(config: Config):
    ...
    from banobot.providers.custom_provider import CustomProvider
    ...
    from banobot.providers.registry import find_by_name
...
def gateway(
    ...
    from banobot.channels.manager import ChannelManager
    from banobot.session.manager import SessionManager
    from banobot.cron.service import CronService
    from banobot.cron.types import CronJob
    from banobot.heartbeat.service import HeartbeatService
...
def channels_status():
    """Show channel status."""
    from banobot.config.loader import load_config
...
def cron_list(
    ...
    """List scheduled jobs."""
    from banobot.config.loader import get_data_dir
    from banobot.cron.service import CronService
...
def cron_add(
    ...
    from banobot.config.loader import get_data_dir
    from banobot.cron.service import CronService
    from banobot.cron.types import CronSchedule
...
def status():
    ...
    if config_path.exists():
        from banobot.providers.registry import PROVIDERS
...
def banabot_config():
    ...
    from banobot.config.loader import load_config, save_config
    from banobot.cli.config_wizard import config_wizard

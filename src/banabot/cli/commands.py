...
from banabot import __logo__, __version__
from banabot.config.schema import Config

app = typer.Typer(
    name="banabot",
    help=f"{__logo__} banabot - Personal AI Assistant",
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} banabot[/cyan]")
    console.print(body)
    console.print()
...
@app.command()
def onboard():
    """Initialize banabot configuration and workspace."""
    ...
    if typer.confirm("🍌 ¿Configurar Banabot ahora? / Configure Banabot now?", default=True):
        from banabot.cli.config_wizard import config_wizard

        config = load_config()
        config = config_wizard(config)
        save_config(config)
    else:
        console.print("  Configura después con: [cyan]banabot config[/cyan]")
...
def _make_provider(config: Config):
    ...
    from banabot.providers.custom_provider import CustomProvider
...
def gateway(
    ...
    """Start the banabot gateway."""
    from banabot.agent.loop import AgentLoop
    from banabot.bus.queue import MessageBus
    from banabot.channels.manager import ChannelManager
    from banabot.config.loader import get_data_dir, load_config
    from banabot.cron.service import CronService
    from banabot.cron.types import CronJob
    from banabot.heartbeat.service import HeartbeatService
    ...
    console.print(f"{__logo__} Starting banabot gateway on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show banabot runtime logs during chat"
    ),
):
    ...
    if logs:
        logger.enable("banabot")
    else:
        logger.disable("banabot")
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]banabot is thinking...[/dim]", spinner="dots")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".banabot" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # banabot/bridge (installed)
    src_bridge = (
        Path(__file__).parent.parent.parent.parent / "bridge"
    )  # repo root/bridge (dev, src layout)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall banabot-ai")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from banabot.config.loader import get_data_dir
    from banabot.cron.service import CronService
...
def status():
    ...
    console.print(f"{__logo__} banabot Status\n")
    ...
    if config_path.exists():
        from banabot.providers.registry import PROVIDERS
...
def banabot_config():
    ...
    from banabot.cli.config_wizard import config_wizard
    from banabot.config.loader import load_config, save_config

...
class DuckDuckGoBackend(SearchProviderBackend):
    ...
    async def search(self, query: str, count: int) -> str:
        try:
            ...
            return (
                "Error: DuckDuckGo search requires 'ddgs' package. Install with: pip install ddgs"
            )
...
class BraveSearchBackend(SearchProviderBackend):
    ...
    async def search(self, query: str, count: int) -> str:
        if not self.api_key:
            return (
                "Error: Brave Search requires an API key. Get one at https://brave.com/search/api/"
            )
...
class SearXNGBackend(SearchProviderBackend):
    ...
    async def search(self, query: str, count: int) -> str:
        if not self.api_base:
            return (
                "Error: SearXNG requires 'apiBase' in config. Set it to your SearXNG instance URL."
            )
